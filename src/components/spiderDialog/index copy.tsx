...
const Dialog:React.FC<IProps> = props => {
  ...
  const [ isInnerShow, setIsInnerShow ] = useState<boolean>(false);
  ...
  const renderContent = (
      <div className={`${style.root} `} onAnimationEnd={handleAnimationEnd}>
        ...
        <div className={`${style.dialog_wrap} ${style[animationClass]}`}>
          <section className={style.dialog_content} style={{ width: props.width }}>
  ...
  if(!el){
    ...
    return isInnerShow && ReactDOM.createPortal( renderContent, foo);
  } else {
    document.body.appendChild(el);
    return isInnerShow && ReactDOM.createPortal( renderContent, el);
  }
